...
def appended_readout(data) -> list:
    """ This is a shortcut to get direct readout values generated by simulator backend 
    to client. Each acquisition channel should have an integer xarray with readout results 
    with the length equal to the number of shots.  

    # TODO: only for testing 1 qubit simulations workflow
    """


    # TODO: turn them into assertions 
    print( data.meas_level.value )
    print( data.meas_return.value )


    dataset_vars = load_data(data)
    readout_vals = dataset_vars["slot~0/acq~0"]

    vhex = np.vectorize(hex)

    # client expects hex values for get_counts() to work 
    # xarray passed from the storage file
    # only works for one qubit simulations
    readout_hex = [list(vhex(readout_vals[0]))]

    # shortcut to get one channel measurement to client 
    # TODO: inspect what behavior is expected for multiple qubits/ experiments 
    return readout_hex
